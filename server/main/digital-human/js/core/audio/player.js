// 音频播放模块
import BlockingQueue from '../../utils/blocking-queue.js?v=0258';
import { applySpeakerSink } from './devices.js?v=0258';
import { log } from '../../utils/logger.js?v=0205';
import { createStreamingContext } from './stream-context.js?v=0258';

// 音频播放器类
export class AudioPlayer {
    constructor() {
        // 与语音核心下行一致：xiaozhi/Huoshan TTS 默认按 24kHz Opus 发包
        // 若仍按 16kHz 解码，会几乎无声或严重失真
        this.SAMPLE_RATE = 24000;
        this.CHANNELS = 1;
        this.FRAME_SIZE = 1440; // 24000 * 60ms
    ...
    getAudioContext() {
        if (!this.audioContext) {
            ...
            void applySpeakerSink(this.audioContext);
        }
    ...
    async playBufferedAudio() {
        try {
            ...
            if (this.audioContext.state === 'suspended') {
                try { await this.audioContext.resume(); } catch (_) { /* Safari */ }
            }
    ...
    enqueueAudioData(opusData) {
        if (opusData.length > 0) {
            ...
            // 收到首包时再尝试解锁播放，避免刷新后 AudioContext 一直 suspended
            if (this.audioContext?.state === 'suspended') {
    ...
    // 标记本轮 TTS 发包结束，让播放队列自然播完（禁止在 stop 时 clear）
    markEndOfStream() {
        if (this.streamingContext) {
            this.streamingContext.endOfStream = true;
        }
