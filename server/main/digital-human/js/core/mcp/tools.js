...
async function loadMuseMcpTools() {
    try {
        const res = await fetch('js/config/muse-mcp-tools.json');
        if (!res.ok) return [];
        return await res.json();
    } catch (e) {
        log('加载 Muse MCP 工具失败', 'warning');
        return [];
    }
}

function invokeMuseParentTool(toolName, toolArgs) {
    return new Promise((resolve, reject) => {
        if (!window.parent || window.parent === window) {
            reject(new Error('Muse 父页面不可用'));
            return;
        }
        const requestId = `mcp-${Date.now()}-${Math.random().toString(36).slice(2)}`;
        const timeout = setTimeout(() => {
            window.removeEventListener('message', onMsg);
            reject(new Error('Muse UI 工具调用超时'));
        }, 8000);
        const onMsg = (event) => {
            if (event.origin !== window.location.origin) return;
            const msg = event.data || {};
            if (msg.source !== 'muse-parent' || msg.type !== 'mcp-tool-result' || msg.requestId !== requestId) return;
...
export async function initMcpTools() {
    ...
    if (isMuseMode()) {
        const museTools = await loadMuseMcpTools();
        const byName = new Map(mcpTools.map(t => [t.name, t]));
        museTools.forEach(tool => byName.set(tool.name, tool));
        mcpTools = [...byName.values()];
    }
    renderMcpTools();
...
export async function executeMcpTool(toolName, toolArgs) {
    ...
    if (toolName.startsWith('muse.ui.')) {
        if (!isMuseMode()) {
            return { success: false, error: 'Muse UI 工具仅在 Muse 终端模式下可用' };
        }
        try {
            const result = await invokeMuseParentTool(toolName, toolArgs);
            log(`Muse UI 工具 ${toolName} 执行成功`, 'success');
            return result;
        } catch (e) {
            log(`Muse UI 工具执行失败: ${e.message}`, 'error');
            return { success: false, error: e.message };
        }
    }

    // 处理手势查询工具（Android MediaPipe）
    if (toolName === 'self_get_hand_gesture') {
        try {
            const mod = await import('../../android-bridge.js?v=0101');
            const gesture = mod.getLatestAndroidGesture ? mod.getLatestAndroidGesture() : null;
            if (gesture && Date.now() - gesture.at < 5000) {
                return { success: true, gesture: gesture.name, score: gesture.score, handedness: gesture.handedness };
            }
            return { success: true, gesture: 'None', message: '当前未检测到手势' };
        } catch (e) {
            return { success: false, error: e.message };
        }
    }

    // 处理传感器查询工具（Android 传感器预留，当前无外接传感器）
    if (toolName === 'self_get_sensor_data') {
        if (typeof window.JarvisAndroid === 'undefined') {
            return { success: true, count: 0, sensors: [], message: '传感器仅在 Jarvis Android 端可用' };
        }
        try {
            const sensorId = toolArgs && toolArgs.sensor_id;
            const raw = sensorId
                ? window.JarvisAndroid.readSensorJson(String(sensorId))
                : window.JarvisAndroid.getSensorsJson();
