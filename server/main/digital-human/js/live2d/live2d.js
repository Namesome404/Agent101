...
class Live2DManager {
    ...
    async initializeLive2D() {
        try {
            ...
            // 从 Muse 智能体配置或本地设置读取形象
            const museAvatar = localStorage.getItem('xz_tester_avatar');
            const savedModelName = museAvatar || localStorage.getItem('live2dModel') || 'hiyori_pro_zh';
            if (savedModelName === 'visualizer' || savedModelName === 'default' || savedModelName === 'sound_visualizer') {
                throw new Error('当前为声波可视化模式，无需加载 Live2D');
            }
            const modelFileMap = {
