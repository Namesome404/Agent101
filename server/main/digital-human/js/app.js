// 主应用入口
import { checkOpusLoaded, initOpusEncoder } from './core/audio/opus-codec.js?v=0258';
import { getAudioPlayer } from './core/audio/player.js?v=0264';
import { checkMicrophoneAvailability, isHttpNonLocalhost } from './core/audio/recorder.js?v=0264';
import { initMcpTools } from './core/mcp/tools.js?v=0258';
import { startWakewordBridgeListener } from './core/network/wakeword-bridge.js?v=0205';
import { installMuseBridge, isMuseMode, isRemoteMode, prepareMuseBridge } from './muse-bridge.js?v=0271';
import { installAndroidBridge } from './android-bridge.js?v=0101';
import { resolveAvatarConfig } from './avatar-mode.js?v=0210';
import { SoundVisualizer } from './visualizer/sound-visualizer.js?v=0213';
...
class App {
    ...
    async init() {
        log('正在初始化应用...', 'info');
        await prepareMuseBridge();
        installAndroidBridge();
        this.uiController = uiController;
        this.uiController.init();
        installMuseBridge(this.uiController);

        if (isMuseMode()) {
            const { applySpeakerSink } = await import('./core/audio/devices.js?v=0258');
            window.addEventListener('ev-audio-devices-changed', async () => {
                if (this.audioPlayer?.audioContext) {
                    await applySpeakerSink(this.audioPlayer.audioContext);
                }
            });
        }
        ...
        if (isRemoteMode()) {
            ...
            initOpusEncoder();
            initMcpTools();
            document.getElementById('modelLoading')?.style.setProperty('display', 'none');
            log('远程输入模式已就绪', 'success');
            return;
        }
        ...
        // 远程/iPhone 模式：在用户点击时再申请麦克风，避免 iOS 初始化拒绝
        // EV / 桌面：只做轻量可用性判断，禁止「开麦再立刻关掉」的探测（会反复弹权限）
        if (isRemoteMode()) {
            window.microphoneAvailable = undefined;
            window.cameraAvailable = true;
        } else if (isMuseMode()) {
            window.microphoneAvailable = true;
            window.cameraAvailable = true;
            log('EV 终端：延迟到连接时再申请麦克风，初始化不弹权限', 'info');
        } else {
            await this.checkMicrophoneAvailability();
        }
        // 检查摄像头可用性（仅标记可用，不打开画面）
        this.checkCameraAvailability();
        // 初始化智能体形象（默认声波可视化，可选 Live2D）
        this.avatarConfig = resolveAvatarConfig();
    ...
    // 初始化智能体形象
    async initAvatar() {
        const live2dCanvas = document.getElementById('live2d-stage');
        const vizCanvas = document.getElementById('sound-visualizer-stage');
        try {
            if (this.avatarConfig.mode === 'visualizer') {
                live2dCanvas?.classList.add('hidden');
                vizCanvas?.classList.remove('hidden');
                this.soundVisualizer = new SoundVisualizer();
                await this.soundVisualizer.start();
                this.live2dManager = null;
                log('智能体形象：声波可视化', 'success');
                return;
            }
            ...
            log(`智能体形象初始化失败: ${error.message}`, 'error');
        }
    }
    ...
    async initCamera() {
        ...
        return new Promise((resolve) => {
            window.startCamera = async () => {
                try {
                    if (window.__MUSE_VIDEO_STREAM__?.getVideoTracks().length) {
                        this.cameraStream = window.__MUSE_VIDEO_STREAM__;
                        cameraVideo.srcObject = this.cameraStream;
                        cameraContainer.classList.add('active');
                        log('摄像头已就绪（远程模式）', 'success');
                        return true;
                    }
