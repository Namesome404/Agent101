...
def _upstream_proxy_configured() -> bool:
    """是否配置了出网代理（则本地 DNS 预检无意义，见 is_safe_url 内注释）。"""
    return any(
        os.environ.get(name)
...
def is_safe_url(url: str, verbose: bool = False) -> bool:
    ...
    try:
        ...
        # EV 适配：走上游代理时跳过本地 DNS 预检。
        # 理由有二：①出网请求由代理远端解析，本地解析结果与实际连接无关；
        # ②本机 DNS 被污染，en.wikipedia.org / google.com 会解析到保留地址
        # 2001::1，导致正常站点被 SSRF 防护误判为「内网地址」而全部拦掉。
        # 字面私有 IP 的 URL 仍在上面的分支被拦，localhost/黑名单/可疑 TLD
        # 检查也都保留，防护意图不变。
        if _upstream_proxy_configured():
            return True
        try:
