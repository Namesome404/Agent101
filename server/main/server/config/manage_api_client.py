...
def _memory_mac_path(mac_address: str) -> str:
    """MAC 含冒号时需编码，否则 URL 可能被错误解析。"""
    return quote(mac_address or "", safe="")


async def save_agent_memory(mac_address: str, summary_memory: str) -> Optional[Dict]:
    """把本地总结好的记忆写回管理端（按设备 MAC 定位智能体）"""
    if not mac_address or not ManageApiClient._instance:
        return None
    try:
        await ManageApiClient._instance._execute_async_request(
            "PUT",
            f"/agent/saveMemory/{_memory_mac_path(mac_address)}",
            json={"summaryMemory": summary_memory or ""},
        )
        return {"ok": True}
    except Exception as e:
        print(f"保存智能体记忆失败: {e}")
...
async def append_agent_memory_items(
    mac_address: str, texts: List[str], source: str = "explicit"
) -> Optional[Dict]:
    """会话中即时追加记忆条目。"""
    if not mac_address or not ManageApiClient._instance:
        return None
    ...
    try:
        await ManageApiClient._instance._execute_async_request(
            "POST",
            f"/agent/appendMemory/{_memory_mac_path(mac_address)}",
            json={"items": cleaned, "source": source},
        )
        return {"ok": True}
    except Exception as e:
        print(f"追加智能体记忆失败: {e}")
