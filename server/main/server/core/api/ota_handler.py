...
class OTAHandler(BaseHandler):
    ...
    def _get_websocket_url(self, request, local_ip: str, port: int) -> str:
        """获取下发给设备的 websocket 地址（跟随访问来源）。

        优先级：
        1) 经反向代理访问（iPhone 走 Tailscale/Caddy、将来云端域名）——用代理带来的
           X-Forwarded-Host + X-Forwarded-Proto 推导，https→wss，同源、换域名即上云。
        2) 配置里写死了明确 websocket 地址（非占位符）——用它。
        3) 局域网直连（ESP32 等）——明文 ws://本机IP:ws端口/。
        """
        # 1) 反向代理来源：跟随转发头
        try:
            ...
            if fwd_host:
                ...
                scheme = "wss" if (fwd_proto or "https").split(",")[0].strip().lower() == "https" else "ws"
                return f"{scheme}://{fwd_host}/xiaozhi/v1/"
        except Exception as e:
            self.logger.bind(tag=TAG).warning(f"解析转发头失败,回退默认ws地址: {e}")
