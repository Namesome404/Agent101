...
class WebSocketServer:
    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        client_id = headers.get("client-id") or ""
        if not device_id:
            ...
            client_id = (query_params.get("client-id") or [client_id])[0]
            if client_id:
                websocket.request.headers["client-id"] = client_id
        self.logger.bind(tag=TAG).info(
            f"WS incoming device={device_id} client={client_id} path={websocket.request.path}"
        )
        if not device_id:
        ...
        if not client_id:
            ...
            parsed_url = urlparse(request_path)
            query_params = parse_qs(parsed_url.query)
            client_id = (query_params.get("client-id") or [""])[0]
            if client_id:
                websocket.request.headers["client-id"] = client_id
        ...
        from core.muse_session_hub import (
            is_remote_client,
            parse_muse_agent_id,
            relay_remote_connection,
        )
        agent_id = parse_muse_agent_id(device_id)
        if agent_id and is_remote_client(client_id):
            self.logger.bind(tag=TAG).info(
                f"Muse remote WS agent={agent_id} client={client_id} device={device_id}"
            )
            try:
                await relay_remote_connection(websocket, agent_id)
            except Exception as e:
                self.logger.bind(tag=TAG).error(f"Muse remote relay error: {e}")
            finally:
                try:
                    await websocket.close()
                except Exception:
