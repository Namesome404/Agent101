...
def get_vision_url_for_headers(config: dict, headers=None) -> str:
    """跟随反向代理 Host 推导 vision URL（iPhone HTTPS / 云端域名）。"""
    headers = headers or {}
    try:
        ...
        if fwd_host:
            ...
            scheme = (
                "https"
                if (fwd_proto or "https").split(",")[0].strip().lower() == "https"
                else "http"
            )
            return f"{scheme}://{fwd_host}/mcp/vision/explain"
    except Exception:
        pass
