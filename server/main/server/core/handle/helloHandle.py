...
async def handleHelloMessage(conn: "ConnectionHandler", msg_json):
    ...
    if audio_params:
        ...
        # 与客户端声明的下行采样率对齐，避免 TTS Opus(24k) 被前端按 16k 播成静音
        try:
            sr = int(audio_params.get("sample_rate") or 0)
            if sr in (8000, 16000, 24000, 48000):
                conn.sample_rate = sr
                if getattr(conn, "tts", None) is not None:
                    if hasattr(conn.tts, "audio_params") and isinstance(conn.tts.audio_params, dict):
                        conn.tts.audio_params["sample_rate"] = sr
                conn.logger.bind(tag=TAG).info(f"按客户端 hello 更新输出采样率: {sr}")
        except Exception:
            pass
