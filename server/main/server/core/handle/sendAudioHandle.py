...
async def _broadcast_muse_tts(conn: "ConnectionHandler", message: dict) -> None:
    """TTS 状态同步到 iPhone 远程端，便于暂停麦克风防回声。"""
    try:
        from core.muse_session_hub import (
            broadcast_to_remotes,
            is_remote_client,
            parse_muse_agent_id,
        )

        agent_id = parse_muse_agent_id(conn.device_id)
        client_id = (conn.headers or {}).get("client-id", "")
        if agent_id and not is_remote_client(client_id):
            await broadcast_to_remotes(agent_id, message)
    except Exception:
        pass
...
async def send_tts_message(conn: "ConnectionHandler", state, text=None):
    ...
    if text is not None:
        ...
        # 记录近期 TTS 文本，用于过滤「识别到自己说话」的回声
        if state == "sentence_start" and text.strip():
            recent = getattr(conn, "recent_tts_texts", None)
            if recent is None:
                recent = []
                conn.recent_tts_texts = recent
    ...
    if state == "stop":
        ...
        # 远程麦场景：播完后多丢一会儿尾音，避免「说完立刻被自己的回声再触发一轮」
        cooldown = 0.45
        try:
            from core.muse_session_hub import has_active_remotes, parse_muse_agent_id

            agent_id = parse_muse_agent_id(conn.device_id)
            if agent_id and has_active_remotes(agent_id):
                cooldown = 0.9
        except Exception:
            pass
        conn.asr_cooldown_until = time.time() + cooldown

    # 发送消息到客户端
    await conn.websocket.send(json.dumps(message))
    await _broadcast_muse_tts(conn, message)
...
async def send_stt_message(conn: "ConnectionHandler", text):
    ...
    payload = {"type": "stt", "text": stt_text, "session_id": conn.session_id}
    ...
    try:
        ...
        client_id = (conn.headers or {}).get("client-id", "")
        if agent_id and not is_remote_client(client_id):
            asyncio.create_task(broadcast_to_remotes(agent_id, payload))
    except Exception:
        pass
    await send_tts_message(conn, "start")
