...
class ConnectionHandler:
    ...
    async def handle_connection(self, ws: websockets.ServerConnection):
        try:
            ...
            # 认证通过,继续处理
            self.websocket = ws
            self._register_muse_host_if_needed()
            ...
            self._unregister_muse_host_if_needed()
            try:
    ...
    async def _route_message(self, message):
        ...
        if isinstance(message, str):
            ...
            # 远程麦克风在线时，忽略主机本地麦克风，避免双路音频导致 ASR 重复
            from core.muse_session_hub import (
                has_active_remotes,
                is_remote_client,
                parse_muse_agent_id,
            )

            agent_id = parse_muse_agent_id(self.device_id)
            client_id = (self.headers or {}).get("client-id", "")
            if (
                agent_id
                and not is_remote_client(client_id)
                and has_active_remotes(agent_id)
            ):
                return
    ...
    def _initialize_components(self):
        try:
            ...
            """加载意图识别（须在记忆之前，记忆配置缺 llm 时也不影响工具）"""
    ...
    def _register_muse_host_if_needed(self):
        from core.muse_session_hub import (
            is_remote_client,
            parse_muse_agent_id,
            register_host,
        ...
        agent_id = parse_muse_agent_id(self.device_id)
        client_id = (self.headers or {}).get("client-id", "")
        if agent_id and not is_remote_client(client_id):
            register_host(agent_id, self)

    def _unregister_muse_host_if_needed(self):
        from core.muse_session_hub import (
            is_remote_client,
            parse_muse_agent_id,
            unregister_host,
    ...
    def _inject_tool_call_fewshot(self):
        ...
        if not hasattr(self, "func_handler") or self.func_handler is None:
            return
        if getattr(self, "_tool_call_fewshot_done", False):
            return
        ...
        self._tool_call_fewshot_done = True
        self.logger.bind(tag=TAG).debug("已注入工具调用 few-shot 示例")

    def _inject_muse_skill_fewshot(self):
        """Muse 终端：教模型真正调用天气/新闻插件，勿口头假扮「已调用工具」。"""
        if not str(self.device_id or "").startswith("muse:"):
            return
        if self.intent_type != "function_call":
            return
        if not hasattr(self, "func_handler") or self.func_handler is None:
            return
        if getattr(self, "_muse_skill_fewshot_done", False):
            return
        ...
        if "get_weather" in tool_names:
            tc_id = "fewshot_muse_weather_001"
            self.dialogue.put(
                Message(role="user", content="今天天气怎么样", is_temporary=True)
            )
            self.dialogue.put(
                Message(
                    role="assistant",
            ...
            self.dialogue.put(
                Message(
                    role="assistant",
                    content="今天不错，20度左右，挺适合出门。",
                    is_temporary=True,
                )
            )

        if "get_news_from_newsnow" in tool_names:
            tc_id = "fewshot_muse_news_001"
            self.dialogue.put(
                Message(role="user", content="看看新闻", is_temporary=True)
            )
            self.dialogue.put(
                Message(
                    role="assistant",
            ...
            self.dialogue.put(
                Message(
                    role="assistant",
                    content="热点给你找好了，挑感兴趣的问我。",
                    is_temporary=True,
                )
            )

            tc_id2 = "fewshot_muse_news_sum_001"
            self.dialogue.put(
                Message(role="user", content="总结一下第五条", is_temporary=True)
            )
            self.dialogue.put(
                Message(
                    role="assistant",
                    tool_calls=[
                        {
                            ...
                            "function": {
                                "name": "get_news_from_newsnow",
                                "arguments": '{"lang":"zh_CN","detail":true,"index":5}',
                            },
                            "type": "function",
                            "index": 0,
                        }
            ...
            tc_id3 = "fewshot_muse_news_detail_001"
            self.dialogue.put(
                Message(role="user", content="看看详细内容", is_temporary=True)
            )
            self.dialogue.put(
                Message(
                    role="assistant",
                    tool_calls=[
                        {
                            ...
                            "function": {
                                "name": "get_news_from_newsnow",
                                "arguments": '{"lang":"zh_CN","detail":true}',
                            },
                            "type": "function",
                            "index": 0,
                        }
            ...
            self.dialogue.put(
                Message(
                    role="assistant",
                    content="正文给你打开了，我简单说下要点。",
                    is_temporary=True,
                )
            )

        self._muse_skill_fewshot_done = True
        self.logger.bind(tag=TAG).debug("已注入 Muse 技能 few-shot 示例")

    def _inject_muse_panel_fewshot(self):
        """Muse 面板 MCP 工具就绪后注入示例，避免「打开窗户」被误解为智能家居通风。"""
        if self.intent_type != "function_call":
            return
        if not hasattr(self, "func_handler") or self.func_handler is None:
            return
        if getattr(self, "_muse_panel_fewshot_done", False):
            return

        tools = self.func_handler.get_functions()
        tool_names = {t.get("function", {}).get("name") for t in (tools or [])}
        if "muse_ui_open_panel" not in tool_names:
            return

        self._muse_panel_fewshot_done = True
        tc_id = "fewshot_muse_panel_001"
        self.dialogue.put(Message(role="user", content="打开窗户", is_temporary=True))
        self.dialogue.put(
            Message(
                role="assistant",
                tool_calls=[
                    {
                        "id": tc_id,
                        "function": {
                            "arguments": '{"panel": "status", "title": "状态", "position": "right"}',
                            "name": "muse_ui_open_panel",
                        },
                        "type": "function",
                        "index": 0,
                    }
        ...
        self.dialogue.put(
            Message(
                role="tool",
                tool_call_id=tc_id,
                content='{"success": true, "message": "已打开状态面板"}',
                is_temporary=True,
            )
        )
        self.dialogue.put(
            Message(
                role="assistant",
                content="好的，已在屏幕上打开状态面板啦。",
                is_temporary=True,
            )
        )

        tc_id2 = "fewshot_muse_web_001"
        self.dialogue.put(
            Message(role="user", content="预览第一条新闻", is_temporary=True)
        )
        self.dialogue.put(
            Message(
                role="assistant",
                tool_calls=[
                    {
                        "id": tc_id2,
                        "function": {
                            "arguments": '{"panel": "web", "title": "新闻预览", "url": "https://www.thepaper.cn/", "position": "right-top", "width": 680, "height": 540}',
                            "name": "muse_ui_open_panel",
                        },
                        "type": "function",
                        "index": 0,
        ...
        self.dialogue.put(
            Message(
                role="tool",
                tool_call_id=tc_id2,
                content='{"success": true, "message": "已打开网页预览"}',
                is_temporary=True,
            )
        )
        self.dialogue.put(
        ...
        self.logger.bind(tag=TAG).debug("已注入 Muse 面板工具 few-shot 示例")
    ...
    async def _initialize_private_config_async(self):
        ...
        if private_config.get("Intent", None) is not None:
            ...
            if model_intent != "Intent_nointent":
                if private_config.get("plugins", None) is not None:
                    plugin_from_server = dict(private_config.get("plugins") or {})
                    parsed_plugins = {}
                    for plugin, config_val in plugin_from_server.items():
                        if isinstance(config_val, dict):
                            parsed_plugins[plugin] = config_val
                        elif isinstance(config_val, str):
                            try:
                                parsed_plugins[plugin] = (
                                    json.loads(config_val) if config_val.strip() else {}
                                )
                            except json.JSONDecodeError:
                                parsed_plugins[plugin] = {}
                        else:
                            parsed_plugins[plugin] = {}
                    base_plugins = dict(self.config.get("plugins") or {})
                    for code, cfg in parsed_plugins.items():
                        if isinstance(base_plugins.get(code), dict) and isinstance(
                            cfg, dict
                        ):
                            base_plugins[code] = {**base_plugins[code], **cfg}
                        else:
                            base_plugins[code] = cfg
                    self.config["plugins"] = base_plugins
                    intent_key = self.config["selected_module"]["Intent"]
                    self.config["Intent"][intent_key]["functions"] = list(
                        parsed_plugins.keys()
                    )
    ...
    def _initialize_memory(self):
        ...
        # 使用 mem_local_short 模式
            ...
            mem_blk = memory_config.get(
                self.config["selected_module"]["Memory"], {}
            )
            memory_llm_name = mem_blk.get("llm")
    ...
    def _initialize_intent(self):
        ...
        # 使用 intent_llm 模式
            ...
            intent_blk = intent_config.get(
                self.config["selected_module"]["Intent"], {}
            )
            intent_llm_name = intent_blk.get("llm")
        ...
        self._inject_muse_skill_fewshot()
    ...
    def chat(self, query, depth=0):
        ...
        if query is not None:
            ...
            if (
                ...
                and hasattr(self.memory, "remember_explicit")
                and depth == 0
            ):
                def remember_explicit_task():
                    try:
                        loop = asyncio.new_event_loop()
                        asyncio.set_event_loop(loop)
                        loop.run_until_complete(self.memory.remember_explicit(query))
                    except Exception as e:
                        self.logger.bind(tag=TAG).warning(f"即时记忆写入失败: {e}")
                    finally:
                        try:
                            loop.close()
                        except Exception:
                ...
                threading.Thread(target=remember_explicit_task, daemon=True).start()

        # 为最顶层时新建会话ID和发送FIRST请求
        if depth == 0:
            current_sentence_id = str(uuid.uuid4().hex)
            self.sentence_id = current_sentence_id  # 更新共享属性
            self._tts_paren_open = None
            self._tts_filter_buf = ""
            try:
                from core.muse_session_hub import (
                    parse_muse_agent_id,
                ...
                muse_agent_id = parse_muse_agent_id(self.device_id)
                if muse_agent_id:
                    sync_shared_history(self, muse_agent_id)
                    publish_shared_message(self, muse_agent_id, "user", query)
            except Exception as e:
                self.logger.bind(tag=TAG).debug(f"Muse 共享会话同步跳过: {e}")
            ...
            for response in llm_responses:
                        ...
                        if tc["name"] == "direct_answer" and tc.get("arguments"):
                            ...
                            if da_text and len(da_text) > sent_len:
                                ...
                                if safe_end > sent_len:
                                    ...
                                    new_part = self._filter_tts_content(new_part)
                                    if new_part:
                                        tc["_da_sent"] = safe_end
                                        self._put_tts_text(current_sentence_id, new_part)
                else:
                    content = response
                ...
                if content is not None and len(content) > 0:
                    if not tool_call_flag:
                        ...
                        tts_part = self._filter_tts_content(content)
                        if tts_part:
                            self._put_tts_text(current_sentence_id, tts_part)
        except Exception as e:
                ...
                if direct_answer_calls:
                    ...
                    for tc in direct_answer_calls:
                        ...
                        if da_response:
                            ...
                            if remaining:
                                remaining = self._filter_tts_content(remaining)
                                if remaining:
                                    self._put_tts_text(current_sentence_id, remaining)
                            # 写入对话历史
                            ...
                            self._publish_muse_conversation_reply(da_response, depth)
        ...
        if len(response_message) > 0:
            ...
            self._publish_muse_conversation_reply(text_buff, depth)
    ...
    def _publish_muse_conversation_reply(self, text, depth=0):
        ...
        try:
            ...
            agent_id = parse_muse_agent_id(self.device_id)
            if agent_id:
                publish_shared_message(self, agent_id, "assistant", text)
        except Exception as e:
            self.logger.bind(tag=TAG).debug(f"Muse 共享回复写入跳过: {e}")

    def _schedule_muse_panel(self, panel_payload):
        """Muse 终端：技能插件结果自动推到浮动窗口（不阻塞语音）。"""
        if not panel_payload or not str(self.device_id or "").startswith("muse:"):
            return
        if not getattr(self, "loop", None):
            return
        try:
            asyncio.run_coroutine_threadsafe(
                self._push_muse_panel_async(panel_payload), self.loop
            )
        except Exception as e:
            self.logger.bind(tag=TAG).warning(f"Muse 窗口上屏失败: {e}")

    async def _push_muse_panel_async(self, panel_payload):
        from core.providers.tools.device_mcp.mcp_handler import call_mcp_tool

        ready = False
        for attempt in range(4):
            if hasattr(self, "mcp_client") and self.mcp_client:
                if await self.mcp_client.is_ready():
                    ready = True
                    break
            if attempt < 3:
        ...
        if not ready:
            self.logger.bind(tag=TAG).warning(
                "Muse 窗口上屏跳过：MCP 未就绪（已重试）"
            )
            return
        tool_name = (
            "muse_ui_update_panel"
            if panel_payload.get("update")
            else "muse_ui_open_panel"
        )
        if not self.mcp_client.has_tool(tool_name):
            tool_name = "muse_ui_open_panel"
            if not self.mcp_client.has_tool(tool_name):
                self.logger.bind(tag=TAG).warning("Muse 窗口上屏跳过：无 open_panel 工具")
                return

        args = {
            "panel": panel_payload.get("panel") or "custom",
            "title": panel_payload.get("title") or "",
            "position": panel_payload.get("position") or "right-top",
            "width": int(panel_payload.get("width") or 440),
            "height": int(panel_payload.get("height") or 380),
        }
        if panel_payload.get("url"):
            args["url"] = panel_payload["url"]
        if panel_payload.get("content"):
            args["content"] = panel_payload["content"]
        if panel_payload.get("data") is not None:
            args["data"] = panel_payload["data"]

        try:
            import sys
            from pathlib import Path

            muse_dir = Path(__file__).resolve().parents[2] / "EV"
            if str(muse_dir) not in sys.path:
                sys.path.insert(0, str(muse_dir))
            from tools.panel import enrich_muse_panel_args

            args = enrich_muse_panel_args(self, args)
        except Exception as e:
            self.logger.bind(tag=TAG).debug(f"Muse 面板补全跳过: {e}")
        ...
        if (
            ...
            try:
                ...
                if str(muse_dir) not in sys.path:
                    sys.path.insert(0, str(muse_dir))
                from tools import web_reader as _wr

                article = _wr.extract_reader(args["url"])
                if article.get("ok"):
                    args["data"] = article
            except Exception as e:
                self.logger.bind(tag=TAG).debug(f"预览正文预抓取跳过: {e}")

        await call_mcp_tool(self, self.mcp_client, tool_name, args, timeout=12)
        self.logger.bind(tag=TAG).info(
            f"Muse 窗口已上屏: panel={args.get('panel')} title={args.get('title')}"
        )
    ...
    def _handle_function_result(self, tool_results, depth, streamed_text=""):
        ...
        for result, tool_call_data in tool_results:
            panel_payload = getattr(result, "panel", None)
            if panel_payload:
                self._schedule_muse_panel(panel_payload)
    ...
    def _put_tts_text(self, sentence_id, content):
        """送文本到 TTS 队列（已过滤括号旁白）。"""
        if not content:
            return
        self.tts.tts_text_queue.put(
    ...
    def _filter_tts_content(self, chunk: str) -> str:
        """过滤 TTS 文本：去除括号内旁白，支持流式分片。"""
        if not chunk:
            return ""
        buf = (getattr(self, "_tts_filter_buf", "") or "") + chunk
        out = []
        paren_open = getattr(self, "_tts_paren_open", None)

        while buf:
            if paren_open:
                close = "）" if paren_open == "（" else ")"
                idx = buf.find(close)
                if idx < 0:
                    self._tts_filter_buf = buf
                    self._tts_paren_open = paren_open
                    break
                buf = buf[idx + 1 :]
                paren_open = None
                continue
            ...
            m = re.search(r"[（(]", buf)
            if not m:
                cleaned = self._clean_response_garbage(buf)
                if cleaned:
                    out.append(cleaned)
                buf = ""
                break
            ...
            before = buf[: m.start()]
            if before:
                cleaned = self._clean_response_garbage(before)
                if cleaned:
                    out.append(cleaned)
            paren_open = m.group()
            buf = buf[m.start() + 1 :]
    ...
    def _clean_response_garbage(text):
        ...
        if not text:
            return text
        # 清理完整括号块（工具旁白、舞台指示等，不应朗读）
        text = re.sub(r"[（(][^）)]*[）)]", "", text)
        text = re.sub(r"[（(]\s*调用[^）)]*[）)]", "", text)
        text = re.sub(r"调用工具[：:,，]?", "", text)
