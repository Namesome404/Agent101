...
class VADProvider(VADProviderBase):
    ...
    def _effective_thresholds(self, conn):
        threshold = self.vad_threshold
        threshold_low = self.vad_threshold_low
        if getattr(conn, "client_is_speaking", False) and conn.client_listen_mode != "manual":
            # TTS 播放中抬高门槛：减弱扬声器回声误触，用户正常说话仍可打断
            threshold = min(0.85, threshold + 0.15)
            threshold_low = min(threshold - 0.05, threshold_low + 0.12)
    ...
    def is_vad(self, conn, opus_packet):
        ...
        try:
            ...
            while len(conn.client_audio_buffer) >= 512 * 2:
                ...
                threshold, threshold_low = self._effective_thresholds(conn)
