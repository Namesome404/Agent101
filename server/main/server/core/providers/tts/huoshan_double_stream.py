...
class TTSProvider(TTSProviderBase):
    ...
    async def _ensure_connection(self):
        ...
        async with self._connection_lock:
            return await self._ensure_connection_unlocked()

    def _ws_alive(self):
        """判断当前复用的 ws 是否仍然可用（避免复用已被服务端关闭的死连接）"""
        ws = self.ws
        if ws is None:
            return False
    ...
    async def _ensure_connection_unlocked(self):
        """建立新的WebSocket连接，并启动监听任务（仅第一次）"""
        try:
            # 复用连接前先探活：火山会在会话空闲后关闭连接，若复用到已关闭的 ws，
            # 本轮 start_session 会失败再重连，表现为每句话前的一大段空档。探活失败就丢弃重建。
            if self.ws and self.enable_ws_reuse and self._ws_alive():
                logger.bind(tag=TAG).debug("复用已有连接")
                return self.ws
            if self.ws:
                if self._ws_alive():
                    try:
            ...
            ws_header = self._websocket_headers()
            ...
            connection_response = self.parser_response(await self.ws.recv())
            if connection_response.optional.event != EVENT_ConnectionStarted:
                error_detail = self._response_error_detail(connection_response)
                raise RuntimeError(
                    f"火山TTS连接初始化失败: {error_detail}"
                )
            logger.bind(tag=TAG).debug("火山TTS连接初始化成功")
            ...
            try:
                ...
                if message.sentence_type == SentenceType.FIRST:
                    ...
                    try:
                        ...
                        logger.bind(tag=TAG).error(f"启动TTS会话失败: {e!r}\n{traceback.format_exc()}")
                        continue
                    ...
                    if message.content_detail:
                        try:
                            ...
                            logger.bind(tag=TAG).error(f"发送TTS文本失败(队列): {e!r}\n{traceback.format_exc()}")
                            continue
                ...
                if message.sentence_type == SentenceType.LAST:
                    try:
                        ...
                        logger.bind(tag=TAG).error(f"结束TTS会话失败: {e!r}\n{traceback.format_exc()}")
                        continue
    ...
    async def text_to_speak(self, text, _):
        ...
        try:
            ...
            logger.bind(tag=TAG).error(f"发送TTS文本失败(text_to_speak): {e!r}\n{traceback.format_exc()}")
            if self.ws:
                try:
                    await self.ws.close()
                except:
        ...
        try:
            while not self.conn.stop_event.is_set():
                try:
                    ...
                    if (
                        res.header.message_type == ERROR_INFORMATION
                        or res.optional.event
                        in (EVENT_ConnectionFailed, EVENT_SessionFailed)
                    ):
                        error_detail = self._response_error_detail(res)
                        self.last_error = error_detail
                        logger.bind(tag=TAG).error(
                            f"火山TTS服务端错误: {error_detail}"
                        )
                        self.activate_session = False
                        break
                    ...
                    if self.activate_session and not self.last_error:
                        self.last_error = (
                            "WebSocket连接已关闭: code=%s reason=%s"
    ...
    def parser_response(self, res) -> Response:
        ...
        header.serialization_method = res[2] >> 4
    ...
    def _response_error_detail(response):
        parts = [f"message_type={response.header.message_type}"]
        if response.optional.errorCode:
            parts.append(f"code={response.optional.errorCode}")
        if response.optional.event:
            parts.append(f"event={response.optional.event}")
        detail = response.optional.response_meta_json
        if not detail and response.payload:
            detail = response.payload.decode("utf-8", errors="replace")
        if detail:
            parts.append(str(detail))
        return " ".join(parts) or "服务端未返回错误详情"
        ...
        try:
            ...
            async def _generate_audio():
                ...
                try:
                    ...
                    connection_response = self.parser_response(await ws.recv())
                    if connection_response.optional.event != EVENT_ConnectionStarted:
                        raise RuntimeError(
                            "火山TTS连接初始化失败: "
                            + self._response_error_detail(connection_response)
                        )
                    ...
                    session_response = self.parser_response(await ws.recv())
                    if session_response.optional.event != EVENT_SessionStarted:
                        raise RuntimeError(
                            "火山TTS会话初始化失败: "
                            + self._response_error_detail(session_response)
                        )
                    ...
                    while True:
                        ...
                        if (
                            ...
                            raise RuntimeError(
                                "火山TTS服务端错误: "
                                + self._response_error_detail(res)
                            )
                        if (
                            res.optional.event == EVENT_TTSResponse
                            and res.header.message_type == AUDIO_ONLY_RESPONSE
