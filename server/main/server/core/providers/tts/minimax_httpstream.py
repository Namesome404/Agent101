...
class TTSProvider(TTSProviderBase):
                ...
                for chunk in response.iter_content(chunk_size=None):
                    ...
                    while True:
                        ...
                        try:
                            ...
                            if stream_data.get("status", 1) == 1 and stream_data.get("audio"):
                                self.pcm_buffer.extend(bytes.fromhex(stream_data["audio"]))
    ...
    def to_tts(self, text: str) -> list:
        ...
        try:
            with self.http_session.post(
                self.api_url, data=json.dumps(payload), headers=headers, timeout=(5, 15)
            ) as response:
                if response.status_code != 200:
                    logger.bind(tag=TAG).error(
                        f"TTS请求失败: {response.status_code}, {response.text}"
