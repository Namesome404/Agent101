...
from core.utils.util import get_vision_url, get_vision_url_for_headers, sanitize_tool_name
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                if next_cursor:
                    ...
                    # Muse 面板工具就绪后注入 few-shot，避免「打开窗户」被误解为智能家居
                    if any("muse_ui" in n for n in mcp_client.tools.keys()):
                        if hasattr(conn, "_inject_muse_panel_fewshot"):
                            conn._inject_muse_panel_fewshot()
            return
...
async def send_mcp_initialize_message(conn: "ConnectionHandler"):
    ...
    vision_url = get_vision_url_for_headers(conn.config, conn.headers or {})
