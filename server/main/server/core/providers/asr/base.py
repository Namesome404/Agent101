...
class ASRProviderBase(ABC):
    ...
    async def handle_voice_stop(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        """并行处理ASR识别"""
        if getattr(conn, "_asr_processing", False):
            return
        conn._asr_processing = True
        try:
            await self._handle_voice_stop_inner(conn, asr_audio_task)
        finally:
            conn._asr_processing = False

    async def _handle_voice_stop_inner(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        """并行处理ASR识别"""
        try:
            ...
            if text_len > 0:
                enhanced_text = textUtils.fix_asr_stutter(enhanced_text)
                # 过滤扬声器回声被当成用户输入
                check_text = content_for_length_check
                if isinstance(enhanced_text, str) and enhanced_text.strip().startswith("{"):
                    try:
                        check_text = json.loads(enhanced_text).get(
                            "content", content_for_length_check
                        )
                    except Exception:
                        pass
                if textUtils.is_likely_tts_echo(
                    check_text, getattr(conn, "recent_tts_texts", None)
                ):
                    logger.bind(tag=TAG).info(
                        f"丢弃疑似 TTS 回声识别: {check_text}"
                    )
                    return
